...
bucket.LOGfilename = fullfile(bucket.pathToWearableDataXsens, 'data.log');
...
% --------TIMESTAMP
% The timestamp appears always before the first string 'XsensSuit::'
tmp.XsensSuitIndx = find(strcmp(tmp.file{1, 1}, 'XsensSuit::'));
wearData.nrOfFrames   = length(tmp.XsensSuitIndx);

wearData.timestamp = zeros(wearData.nrOfFrames,1);
for tsIdx = 1 : wearData.nrOfFrames
    wearData.timestamp(tsIdx,1) = str2num(tmp.file{1, 1}{tmp.XsensSuitIndx(tsIdx)-1,1});
end
